Splicingfinder <- function(GTFdb=NULL,txTable=NULL,calGene=NULL,Ncor=1,out.dir=NULL){
    ...
    if (length(final.MXE.cal) != 0){
      ...
      if(length(MXE.cal) != 0){
        MXE.ES.mat <- do.call(rbind,lapply(paste(MXE.cal[,1],MXE.cal[,2],sep="-"),function(total.mem){
          ...
          if (length(MXE.ES) != 0){
            tar.exs <- matrix(rep(c(paste(mem[1:2],collapse="-"),paste(mem[3:4],collapse="-")),nrow(MXE.ES)),nrow=nrow(MXE.ES),byrow=T)
  ...
  txTable <- try(select(GTFdb, keys=names(trans.exon.range), columns=c("TXCHROM","TXNAME","GENEID","TXSTART","TXEND","TXSTRAND"), keytype="TXID"),silent=T)
  ...
  if (length(ES.finl.result) != 0){
    ES.finl.result <- cbind(Index=paste("ES",1:nrow(ES.finl.result),sep=""),ES.finl.result)
  }
  else  ES.finl.result <- matrix("NA")
  if (length(ASS.finl.result) != 0){
    ASS.finl.result <- cbind(Index=paste("ASS",1:nrow(ASS.finl.result),sep=""),ASS.finl.result)
  }
  else  ASS.finl.result <- matrix("NA")
  if (length(IR.finl.result) != 0){
    IR.finl.result <- cbind(Index=paste("IR",1:nrow(IR.finl.result),sep=""),IR.finl.result)
  }
  ...
  if (length(out.dir) != 0){
    ...
    write.table(ES.finl.result,paste(out.dir,"/AS_Pattern/ES_pattern.txt",sep=""),sep='\t',quote=F)
    write.table(ASS.finl.result,paste(out.dir,"/AS_Pattern/ASS_pattern.txt",sep=""),sep='\t',quote=F)
    write.table(IR.finl.result,paste(out.dir,"/AS_Pattern/IR_pattern.txt",sep=""),sep='\t',quote=F)
  }
  return (ASdb)
}
